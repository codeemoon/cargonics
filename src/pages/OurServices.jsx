...
const SERVICES = [
  ...
  },
  {
    ...
    ctaLink: '/network-and-coverage',
    ctaText: 'Explore Network',
    bg: 'linear-gradient(135deg, #0A1B4D 0%, #162E74 100%)',
    darkText: true
  },
  {
    id: 'door',
  ...
  return (
    <div className="overflow-x-hidden" style={{ background: '#f8f9ff' }}>
          ...
          >
            ...
            <nav className="flex-1 space-y-1">
              {[
                ...
                { to: '/our-services', icon: 'local_shipping', label: 'Services', active: true },
                ...
                >
                  ...
                  <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-between">
                    <div>
                      {/* Badge if exists */}
                      {service.badge && (
                        <span className={`inline-block font-bold px-3 py-1 rounded text-[13px] mb-4 ${
                          service.badge === 'Upcoming' ? 'bg-[#000520] text-white' :
                          service.badge === 'Priority Fast' ? 'bg-[#ffdbcc] text-[#a04100]' :
                          'bg-[#F0FDF4] text-[#009b50]'
                        }`}>
                          {service.badge}
                        </span>
                      )}
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        <span className={`material-symbols-outlined`} style={{ fontSize: '36px', color: isDark ? '#ffb693' : '#fe6b00' }}>
                          {service.icon}
                        </span>
                        <h3 className={`font-[Montserrat] text-[24px] md:text-[28px] font-bold ${isDark ? 'text-white' : 'text-[#000520]'}`}>
                          {service.title}
                        </h3>
                      </div>

                      <p className={`text-[16px] leading-[26px] mb-6 ${isDark ? 'text-white/80' : 'text-[#45464f]'}`}>
                        {service.desc}
                      </p>
                      ...
                      <ul className="space-y-3 mb-8">
                        {service.features.map((feat) => (
                          <li key={feat} className={`flex items-center gap-2 text-[15px] ${isDark ? 'text-white/95' : 'text-[#000520]'}`}>
                            <span className="material-symbols-outlined text-[#009b50]" style={{ fontVariationSettings: "'FILL' 1", fontSize: '20px' }}>
                              check_circle
                            </span>
                            {feat}
                          </li>
                    ...
                    {/* CTA Button */}
                    {service.teaser ? (
                      <button className="w-fit text-[#767680] font-bold cursor-not-allowed text-[16px]">
                        {service.ctaText}
                      </button>
                    ) : (
                      <Link 
                        to={service.ctaLink} 
                        className={`w-fit font-bold px-8 py-3 rounded-lg text-[16px] transition-all active:scale-95 ${
                          isDark 
                            ? 'bg-[#fe6b00] text-white hover:shadow-lg' 
                            : 'border-2 border-[#000520] text-[#000520] hover:bg-[#000520] hover:text-white'
                        }`}
                      >
                        {service.ctaText}
                      </Link>
        ...
        <section className="mt-16 px-4 md:px-[48px] max-w-[1280px] mx-auto">
          <div className="rounded-3xl p-12 md:p-20 text-center relative overflow-hidden"
            ...
            <div className="relative z-10">
              <h2 className="font-[Montserrat] text-[24px] md:text-[48px] font-bold text-white mb-6">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                <Link to="/get-quote"
                  className="bg-[#fe6b00] text-white px-10 py-4 rounded-xl font-bold text-lg hover:shadow-2xl hover:-translate-y-1 transition-all active:scale-95">
                  Get Started Now
                </Link>
                <Link to="/contact-us"
                  className="border-2 text-white px-10 py-4 rounded-xl font-bold text-lg hover:bg-white/10 transition-all"
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full z-50 px-4 py-3 flex justify-around border-t shadow-2xl" style={{ background: '#f8f9ff', borderColor: '#c6c5d0' }}>
        ...
        <Link to="/our-services" className="flex flex-col items-center gap-1 text-[#fe6b00]">
