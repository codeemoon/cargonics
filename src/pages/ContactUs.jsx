...
export default function ContactUs() {
  ...
  useEffect(() => {
    if (!mapInstanceRef.current && mapRef.current) {
      ...
      L.marker([30.6428, 76.8003]).addTo(map)
        .bindPopup('Cargonics Express - Zirakpur')
        .openPopup()
          ...
          >
            ...
            <nav className="flex-1 space-y-1">
              {[
                ...
                { to: '/contact-us', icon: 'contact_support', label: 'Contact', active: true },
              ].map((item) => (
                <Link
                  key={item.to}
                  to={item.to}
        ...
        <section className="relative flex items-center overflow-hidden mb-16" style={{ height: '400px' }}>
          <div className="absolute inset-0 z-0">
            <img
              ...
              alt="Diverse team of logistics professionals in a modern glass office having a collaborative discussion"
            ...
            {[
              {
                icon: 'mail',
                title: 'Email Us',
                desc: 'Our support team typically responds within 2 business hours.',
                cta: <a className="text-[#000520] font-bold text-[18px] hover:underline transition-all" href="mailto:info@cargonics.com">info@cargonics.com</a>,
              },
              {
                icon: 'call',
                title: 'Call Center',
                desc: 'Mon-Sat, 9:00 AM to 7:00 PM IST for immediate assistance.',
                cta: <a className="text-[#000520] font-bold text-[18px] hover:underline transition-all" href="tel:+919599196008">+91 9599196008</a>,
              },
              {
                icon: 'location_on',
                title: 'Main Office',
                desc: 'Cabin No 201, SCO No-2, Chaura Bazar 2, Chandigarh-Ambala Highway, Zirakpur, Punjab.',
                cta: <span className="text-[#000520] font-bold text-[15px] block leading-normal">Cabin No 201, SCO No-2, Zirakpur</span>,
              },
            ].map((card) => (
              <div key={card.title} className="glass-card p-8 rounded-xl shadow-md hover:shadow-lg transition-shadow border-l-4 border-[#fe6b00]">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mb-6" style={{ background: '#dce9ff' }}>
                  <span className="material-symbols-outlined text-[#fe6b00]">{card.icon}</span>
                </div>
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      className="w-full px-4 py-3 rounded-lg border outline-none transition-all text-[16px]"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <select
                      className="w-full px-4 py-3 rounded-lg border outline-none transition-all appearance-none bg-white text-[16px] cursor-pointer"
                      style={{ borderColor: '#c6c5d0' }}
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      className="w-full px-4 py-3 rounded-lg border outline-none transition-all text-[16px]"
                      style={{ borderColor: '#c6c5d0' }}
                      id="phone" name="phone" placeholder="+91 99999 99999" type="tel"
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    className="w-full px-4 py-3 rounded-lg border outline-none transition-all text-[16px] resize-none"
                    style={{ borderColor: '#c6c5d0' }}
                    id="message" name="message" placeholder="How can we help you today?" rows={4}
                ...
                <button
                  ...
                  style={{
                    background: submitState === 'sent' ? '#009b50' : '#0a1b4d',
                    boxShadow: '0 10px 30px rgba(10,27,77,0.20)',
                    opacity: submitState === 'sending' ? 0.80 : 1,
                  }}
                  type="submit"
                  disabled={submitState !== 'idle'}
                >
                  {submitState === 'idle' && (
                    <><span>Send Inquiry</span><span className="material-symbols-outlined">send</span></>
                  )}
                  {submitState === 'sending' && (
                    <><span className="material-symbols-outlined animate-spin">sync</span><span>Sending...</span></>
                  )}
                  {submitState === 'sent' && (
                    <><span className="material-symbols-outlined">check_circle</span><span>Message Sent!</span></>
                  )}
                </button>
              </form>
            </div>
...
<div className="w-full rounded-2xl overflow-hidden shadow-xl border" style={{ height: '400px', borderColor: 'rgba(198,197,208,0.30)' }}>
              ...
              <a 
                href="https://www.openstreetmap.org/?mlat=30.6428&mlon=76.8003#map=17/30.6428/76.8003" 
                target="_blank" 
                rel="noreferrer"
                className="text-[#fe6b00] font-bold text-[14px] hover:underline"
              >
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full z-50 px-4 py-3 flex justify-around border-t shadow-2xl" style={{ background: '#f8f9ff', borderColor: '#c6c5d0' }}>
        ...
        <Link to="/contact-us" className="flex flex-col items-center gap-1 text-[#fe6b00]">
